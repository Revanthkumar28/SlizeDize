...
import { RegistrationDetails, validationResults, Country, State, City ,postIndustri,VendorRegistrationForm } from 'src/app/models/interfaces';
...
export class SignUpVendorComponent implements OnInit {
  ...
  signupDetails: VendorRegistrationForm = {
    contactPersonName: "",
    mobileNumber: "",
    typeOfIndustry: "",
    companyName: "",
  ...
  ngOnInit(): void {
    ...
    this.filterData=''
    this.fetchCountry();
    this.HotelService.vendor_industry().subscribe((data)=>{
      console.log(data)
      this.typeOfIndustry =data.TypeOfIndustry;
    })   
  ...
  register() {
    ...
    if(this.signupDetails.po_access == '0' && this.signupDetails.service_access == '0'){
      this.signupDetails.po_access = '1'
    }
      ...
      this.RegisterFormService.verifyUser(this.signupDetails).subscribe((data)=>{
        ...
        if(data.message =='User exists'){
            ...
            if(data.message =='User does not exists'){
              ...
              this.RegisterFormService.vendor_register_first(this.signupDetails).subscribe((res:any)=>{
                console.log(res)
                this.popup = true
                this.loading = false;
  ...
  empty_all_feilds(){
    this.mobileNumber = ''
    this.signupDetails.contactPersonName = ''
    ...
    this.signupDetails.position = ''
    this.signupDetails.po_access = ''
    this.signupDetails.service_access = ''
  }
  ...
  validateInput(): boolean {
    ...
    if(this.signupDetails.contactPersonName.length == 0) {
  ...
  checkedPurchase(event:any,access:any){
    if(access == 'sales'){
      this.signupDetails.po_access = event.target.checked ? '1' : '0'
    }
    if(access == 'service'){
      this.signupDetails.service_access = event.target.checked ? '1' : '0'
    }
  }
